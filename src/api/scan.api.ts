...
export const scanApi = {
  ...
  startScan: async (): Promise<ScanResult> => {
    const response = await apiClient.post<ApiResponse<ScanResult>>('/scan') as any;
    if (!response.data) {
  ...
  getScanStatus: async (): Promise<any> => {
    const response = await apiClient.get<ApiResponse<any>>('/scan/status') as any;
    return response.data;
  },

  // Get real-time progress (polled)
  getScanProgress: async (): Promise<ScanProgress> => {
    const response = await apiClient.get<ApiResponse<ScanProgress>>('/scan/progress') as any;
    if (!response.data) {
